...
class MaintenanceMpwarehouseController extends GetxController {
  ...
  WarehouseStockJSON get trx => _trx;
  ...
  Future<void> getWarehouseStock() async {
    ...
    if (Get.arguments != null) {
      if (Get.arguments['notificationId'] != null) {
        notificationFilter =
            " and lit_warehousestock_v_ID eq ${Get.arguments['notificationId']}";
    ...
    final protocol = GetStorage().read('protocol');
    var url = Uri.parse(
        '$protocol://$ip/api/v1/models/lit_warehousestock_v?\$filter= AD_Client_ID eq ${GetStorage().read('clientid')}&\$skip=${(pagesCount.value - 1) * 100}');
    //print(url);
    var response = await http.get(
      url,
      headers: <String, String>{
    ...
    if (response.statusCode == 200) {
      ...
      _trx = WarehouseStockJSON.fromJson(
