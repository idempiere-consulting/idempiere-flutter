...
class ProductionFilterOrder extends StatefulWidget {
  const ProductionFilterOrder({Key? key}) : super(key: key);

  @override
  State<ProductionFilterOrder> createState() => _ProductionFilterOrderState();
}

class _ProductionFilterOrderState extends State<ProductionFilterOrder> {
  applyFilters() {
    var inputFormat = DateFormat('dd/MM/yyyy');

    if (productId > 0) {
      Get.find<ProductionOrderController>().productFilter =
    ...
    if (docNoFieldController.text != "") {
      Get.find<ProductionOrderController>().docNoFilter =
    ...
    if (dateStartFieldController.text != "") {
      try {
        ...
        Get.find<ProductionOrderController>().dateStartFilter =
            " and DateInvoiced ge '${DateFormat('yyyy-MM-dd').format(date)} 00:00:00'";
        Get.find<ProductionOrderController>().dateStartValue.value =
    ...
    if (dateEndFieldController.text != "") {
      try {
        ...
        Get.find<ProductionOrderController>().dateEndFilter =
            " and DateInvoiced le '${DateFormat('yyyy-MM-dd').format(date)} 23:59:59'";
        Get.find<ProductionOrderController>().dateEndValue.value =
    ...
    Get.find<ProductionOrderController>().productId.value = productId;

    if (productId > 0) {
      Get.find<ProductionOrderController>().productName =
          productSearchFieldController.text;
    } else {
      Get.find<ProductionOrderController>().productName = "";
    }
    Get.find<ProductionOrderController>().docNoValue.value =
        docNoFieldController.text;

    Get.find<ProductionOrderController>().getProductionOrders();
    Get.back();
  }
  ...
  saveFilters() {
    ...
    if (productId > 0) {
      GetStorage().write('ProductionOrder_businessPartnerFilter',
          " and M_Product_ID eq $productId");
    } else {
      GetStorage().write('ProductionOrder_productFilter', "");
    }

    if (docNoFieldController.text != "") {
      GetStorage().write('ProductionOrder_docNoFilter',
          " and contains(DocumentNo,'${docNoFieldController.text}')");
    } else {
      GetStorage().write('ProductionOrder_docNoFilter', "");
    }
    ...
    if (dateStartFieldController.text != "") {
      try {
        var date = inputFormat.parse(dateStartFieldController.text);
        GetStorage().write('ProductionOrder_dateStartFilter',
            " and DateInvoiced ge '${DateFormat('yyyy-MM-dd').format(date)} 00:00:00'");
        GetStorage()
            .write('ProductionOrder_dateStart', dateStartFieldController.text);
      } catch (e) {
        if (kDebugMode) {
          print(e);
        }
      ...
      GetStorage().write('ProductionOrder_dateStartFilter', "");
      GetStorage().write('ProductionOrder_dateStart', "");
    }
    ...
    if (dateEndFieldController.text != "") {
      try {
        ...
        GetStorage().write('ProductionOrder_dateEndFilter',
            " and DateInvoiced le '${DateFormat('yyyy-MM-dd').format(date)} 23:59:59'");
        GetStorage()
            .write('ProductionOrder_dateEnd', dateEndFieldController.text);
      } catch (e) {
        if (kDebugMode) {
          print(e);
        }
      ...
      GetStorage().write('ProductionOrder_dateEndFilter', "");
      GetStorage().write('ProductionOrder_dateEnd', "");
    }
    GetStorage().write('ProductionOrder_productId', productId);
    GetStorage().write(
        'ProductionOrder_productName', productSearchFieldController.text);
    GetStorage().write('ProductionOrder_docNo', docNoFieldController.text);
  }
