...
class PortalMPFilterInvoice extends StatefulWidget {
  const PortalMPFilterInvoice({Key? key}) : super(key: key);

  @override
  State<PortalMPFilterInvoice> createState() => _PortalMPFilterInvoiceState();
}

class _PortalMPFilterInvoiceState extends State<PortalMPFilterInvoice> {
  applyFilters() {
  ...
  saveFilters() {
    ...
    if (selectedUserRadioTile > 0) {
      if (selectedUserRadioTile == 2 && salesRepId > 0) {
        GetStorage().write(
            'PortalMPInvoice_userFilter', " and SalesRep_ID eq $salesRepId");
      } else {
        GetStorage().write('Invoice_userFilter',
            " and SalesRep_ID eq ${GetStorage().read('userId')}");
      }
    } else {
      GetStorage().write('PortalMPInvoice_userFilter', "");
    }

    if (businessPartnerId > 0) {
      GetStorage().write('PortalMPInvoice_businessPartnerFilter',
          " and C_BPartner_ID eq $businessPartnerId");
    } else {
      GetStorage().write('PortalMPInvoice_businessPartnerFilter', "");
    }
    ...
    if (docNoFieldController.text != "") {
      GetStorage().write('PortalMPInvoice_docNoFilter',
          " and contains(DocumentNo,'${docNoFieldController.text}')");
    } else {
      GetStorage().write('PortalMPInvoice_docNoFilter', "");
    }
    ...
    if (descriptionFieldController.text != "") {
      GetStorage().write('PortalMPInvoice_descriptionFilter',
          " and contains(tolower(Description),'${descriptionFieldController.text}')");
    } else {
      GetStorage().write('PortalMPInvoice_descriptionFilter', "");
    }
    ...
    if (dateStartFieldController.text != "") {
      try {
        var date = inputFormat.parse(dateStartFieldController.text);
        GetStorage().write('PortalMPInvoice_dateStartFilter',
            " and DateInvoiced ge '${DateFormat('yyyy-MM-dd').format(date)} 00:00:00'");
        GetStorage()
            .write('PortalMPInvoice_dateStart', dateStartFieldController.text);
      } catch (e) {
        if (kDebugMode) {
          print(e);
        }
      ...
      GetStorage().write('PortalMPInvoice_dateStartFilter', "");
      GetStorage().write('PortalMPInvoice_dateStart', "");
    }
    ...
    if (dateEndFieldController.text != "") {
      try {
        ...
        GetStorage().write('PortalMPInvoice_dateEndFilter',
            " and DateInvoiced le '${DateFormat('yyyy-MM-dd').format(date)} 23:59:59'");
        GetStorage()
            .write('PortalMPInvoice_dateEnd', dateEndFieldController.text);
      } catch (e) {
        if (kDebugMode) {
          print(e);
        }
      ...
      GetStorage().write('PortalMPInvoice_dateEndFilter', "");
      GetStorage().write('PortalMPInvoice_dateEnd', "");
    }

    GetStorage()
        .write('PortalMPInvoice_selectedUserRadioTile', selectedUserRadioTile);
    GetStorage().write('PortalMPInvoice_salesRepId', salesRepId);
    GetStorage()
        .write('PortalMPInvoice_salesRepName', salesRepFieldController.text);
    GetStorage().write('PortalMPInvoice_businessPartnerId', businessPartnerId);
    GetStorage().write(
        'PortalMPInvoice_businessPartnerName', bpSearchFieldController.text);
    GetStorage().write('PortalMPInvoice_docNo', docNoFieldController.text);
    GetStorage()
        .write('PortalMPInvoice_description', descriptionFieldController.text);
  }
