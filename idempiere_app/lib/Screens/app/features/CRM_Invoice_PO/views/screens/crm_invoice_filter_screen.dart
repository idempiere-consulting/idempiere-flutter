...
class CRMFilterInvoicePO extends StatefulWidget {
  const CRMFilterInvoicePO({Key? key}) : super(key: key);

  @override
  State<CRMFilterInvoicePO> createState() => _CRMFilterInvoicePOState();
}

class _CRMFilterInvoicePOState extends State<CRMFilterInvoicePO> {
  applyFilters() {
    var inputFormat = DateFormat('dd/MM/yyyy');
    if (selectedUserRadioTile > 0) {
      if (selectedUserRadioTile == 2 && salesRepId > 0) {
        Get.find<CRMInvoicePOController>().userFilter =
    ...
    if (businessPartnerId > 0) {
      Get.find<CRMInvoicePOController>().businessPartnerFilter =
    ...
    if (docNoFieldController.text != "") {
      Get.find<CRMInvoicePOController>().docNoFilter =
    ...
    if (descriptionFieldController.text != "") {
      Get.find<CRMInvoicePOController>().descriptionFilter =
    ...
    if (dateStartFieldController.text != "") {
      try {
        ...
        Get.find<CRMInvoicePOController>().dateStartFilter =
            " and DateInvoiced ge '${DateFormat('yyyy-MM-dd').format(date)} 00:00:00'";
        Get.find<CRMInvoicePOController>().dateStartValue.value =
    ...
    if (dateEndFieldController.text != "") {
      try {
        ...
        Get.find<CRMInvoicePOController>().dateEndFilter =
            " and DateInvoiced le '${DateFormat('yyyy-MM-dd').format(date)} 23:59:59'";
        Get.find<CRMInvoicePOController>().dateEndValue.value =
    ...
    Get.find<CRMInvoicePOController>().selectedUserRadioTile.value =
        selectedUserRadioTile;

    Get.find<CRMInvoicePOController>().salesRepId = salesRepId;
    Get.find<CRMInvoicePOController>().salesRepName =
        salesRepFieldController.text;

    Get.find<CRMInvoicePOController>().businessPartnerId.value =
        businessPartnerId;

    if (businessPartnerId > 0) {
      Get.find<CRMInvoicePOController>().businessPartnerName =
          bpSearchFieldController.text;
    } else {
      Get.find<CRMInvoicePOController>().businessPartnerName = "";
    }
    Get.find<CRMInvoicePOController>().docNoValue.value =
        docNoFieldController.text;
    Get.find<CRMInvoicePOController>().description.value =
        descriptionFieldController.text;

    Get.find<CRMInvoicePOController>().getInvoices();
    Get.back();
  }
