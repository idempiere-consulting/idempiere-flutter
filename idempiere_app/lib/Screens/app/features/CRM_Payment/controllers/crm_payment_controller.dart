...
class CRMPaymentController extends GetxController {
  ...
  Future<void> getPayments() async {
    ...
    final protocol = GetStorage().read('protocol');
    var url = Uri.parse(
        '$protocol://$ip/api/v1/models/C_Payment?\$filter= IsReceipt eq Y and AD_Client_ID eq ${GetStorage().read("clientid")}${apiUrlFilter[filterCount]}$businessPartnerFilter$dateStartFilter$dateEndFilter$bankAccountFilter&\$orderby= DateAcct desc&\$skip=${(pagesCount.value - 1) * 100}');
    var response = await http.get(
      url,
      headers: <String, String>{
        'Content-Type': 'application/json',
