...
class PurchaseFilterInvoice extends StatefulWidget {
  const PurchaseFilterInvoice({Key? key}) : super(key: key);

  @override
  State<PurchaseFilterInvoice> createState() => _PurchaseFilterInvoiceState();
}

class _PurchaseFilterInvoiceState extends State<PurchaseFilterInvoice> {
  applyFilters() {
    var inputFormat = DateFormat('dd/MM/yyyy');
    if (selectedUserRadioTile > 0) {
      if (selectedUserRadioTile == 2 && salesRepId > 0) {
        Get.find<PurchaseInvoiceController>().userFilter =
    ...
    if (businessPartnerId > 0) {
      Get.find<PurchaseInvoiceController>().businessPartnerFilter =
    ...
    if (docNoFieldController.text != "") {
      Get.find<PurchaseInvoiceController>().docNoFilter =
    ...
    if (descriptionFieldController.text != "") {
      Get.find<PurchaseInvoiceController>().descriptionFilter =
    ...
    if (dateStartFieldController.text != "") {
      try {
        ...
        Get.find<PurchaseInvoiceController>().dateStartFilter =
            " and DateInvoiced ge '${DateFormat('yyyy-MM-dd').format(date)} 00:00:00'";
        Get.find<PurchaseInvoiceController>().dateStartValue.value =
    ...
    if (dateEndFieldController.text != "") {
      try {
        ...
        Get.find<PurchaseInvoiceController>().dateEndFilter =
            " and DateInvoiced le '${DateFormat('yyyy-MM-dd').format(date)} 23:59:59'";
        Get.find<PurchaseInvoiceController>().dateEndValue.value =
    ...
    Get.find<PurchaseInvoiceController>().selectedUserRadioTile.value =
        selectedUserRadioTile;

    Get.find<PurchaseInvoiceController>().salesRepId = salesRepId;
    Get.find<PurchaseInvoiceController>().salesRepName =
        salesRepFieldController.text;

    Get.find<PurchaseInvoiceController>().businessPartnerId.value =
        businessPartnerId;

    if (businessPartnerId > 0) {
      Get.find<PurchaseInvoiceController>().businessPartnerName =
          bpSearchFieldController.text;
    } else {
      Get.find<PurchaseInvoiceController>().businessPartnerName = "";
    }
    Get.find<PurchaseInvoiceController>().docNoValue.value =
        docNoFieldController.text;
    Get.find<PurchaseInvoiceController>().description.value =
  ...
  saveFilters() {
    ...
    if (selectedUserRadioTile > 0) {
      if (selectedUserRadioTile == 2 && salesRepId > 0) {
        GetStorage().write(
            'PurchaseInvoice_userFilter', " and SalesRep_ID eq $salesRepId");
      } else {
        GetStorage().write('Invoice_userFilter',
            " and SalesRep_ID eq ${GetStorage().read('userId')}");
      }
    } else {
      GetStorage().write('PurchaseInvoice_userFilter', "");
    }

    if (businessPartnerId > 0) {
      GetStorage().write('PurchaseInvoice_businessPartnerFilter',
          " and C_BPartner_ID eq $businessPartnerId");
    } else {
    ...
    if (docNoFieldController.text != "") {
      GetStorage().write('PurchaseInvoice_docNoFilter',
          " and contains(DocumentNo,'${docNoFieldController.text}')");
    } else {
      GetStorage().write('PurchaseInvoice_docNoFilter', "");
    }
    ...
    if (descriptionFieldController.text != "") {
      GetStorage().write('PurchaseInvoice_descriptionFilter',
          " and contains(tolower(Description),'${descriptionFieldController.text}')");
    } else {
      GetStorage().write('PurchaseInvoice_descriptionFilter', "");
    }
    ...
    if (dateStartFieldController.text != "") {
      try {
        var date = inputFormat.parse(dateStartFieldController.text);
        GetStorage().write('PurchaseInvoice_dateStartFilter',
            " and DateInvoiced ge '${DateFormat('yyyy-MM-dd').format(date)} 00:00:00'");
        GetStorage()
            .write('PurchaseInvoice_dateStart', dateStartFieldController.text);
      } catch (e) {
        if (kDebugMode) {
          print(e);
        }
      ...
      GetStorage().write('PurchaseInvoice_dateStartFilter', "");
      GetStorage().write('PurchaseInvoice_dateStart', "");
    }
    ...
    if (dateEndFieldController.text != "") {
      try {
        ...
        GetStorage().write('PurchaseInvoice_dateEndFilter',
            " and DateInvoiced le '${DateFormat('yyyy-MM-dd').format(date)} 23:59:59'");
        GetStorage()
            .write('PurchaseInvoice_dateEnd', dateEndFieldController.text);
      } catch (e) {
        if (kDebugMode) {
          print(e);
        }
      ...
      GetStorage().write('PurchaseInvoice_dateEndFilter', "");
      GetStorage().write('PurchaseInvoice_dateEnd', "");
    }

    GetStorage()
        .write('PurchaseInvoice_selectedUserRadioTile', selectedUserRadioTile);
    GetStorage().write('PurchaseInvoice_salesRepId', salesRepId);
    GetStorage()
        .write('PurchaseInvoice_salesRepName', salesRepFieldController.text);
    GetStorage().write('PurchaseInvoice_businessPartnerId', businessPartnerId);
    GetStorage().write(
        'PurchaseInvoice_businessPartnerName', bpSearchFieldController.text);
    GetStorage().write('PurchaseInvoice_docNo', docNoFieldController.text);
    GetStorage()
        .write('PurchaseInvoice_description', descriptionFieldController.text);
  }
