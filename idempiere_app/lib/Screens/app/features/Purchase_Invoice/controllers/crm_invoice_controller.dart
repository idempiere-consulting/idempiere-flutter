...
class PurchaseInvoiceController extends GetxController {
  ...
  var userFilter = GetStorage().read('PurchaseInvoice_userFilter') ?? "";
  var businessPartnerFilter =
      GetStorage().read('PurchaseInvoice_businessPartnerFilter') ?? "";
  var docNoFilter = GetStorage().read('PurchaseInvoice_docNoFilter') ?? "";
  var descriptionFilter =
      GetStorage().read('PurchaseInvoice_descriptionFilter') ?? "";
  var dateStartFilter =
      GetStorage().read('PurchaseInvoice_dateStartFilter') ?? "";
  var dateEndFilter = GetStorage().read('PurchaseInvoice_dateEndFilter') ?? "";
  ...
  void onInit() {
    ...
    selectedUserRadioTile.value =
        GetStorage().read('PurchaseInvoice_selectedUserRadioTile') ?? 0;
    businessPartnerName =
        GetStorage().read('PurchaseInvoice_businessPartnerName') ?? "";
    businessPartnerId.value =
        GetStorage().read('PurchaseInvoice_businessPartnerId') ?? 0;
    salesRepId = GetStorage().read('PurchaseInvoice_salesRepId') ?? 0;
    salesRepName = GetStorage().read('PurchaseInvoice_salesRepName') ?? "";
    docNoValue.value = GetStorage().read('PurchaseInvoice_docNo') ?? "";
    description.value = GetStorage().read('PurchaseInvoice_description') ?? "";
    dateStartValue.value = GetStorage().read('PurchaseInvoice_dateStart') ?? "";
    dateEndValue.value = GetStorage().read('PurchaseInvoice_dateEnd') ?? "";
    getInvoices();
    getADUserID();
    setConnect();
  }
  ...
  Future<void> getInvoices() async {
    ...
    final protocol = GetStorage().read('protocol');
    var url = Uri.parse(
        '$protocol://$ip/api/v1/models/c_invoice?\$filter= IsSoTrx eq N and AD_Client_ID eq ${GetStorage().read("clientid")}$notificationFilter$userFilter$businessPartnerFilter$docNoFilter$descriptionFilter$dateStartFilter$dateEndFilter&\$orderby= DateInvoiced desc&\$skip=${(pagesCount.value - 1) * 100}');
    var response = await http.get(
      url,
      headers: <String, String>{
        'Content-Type': 'application/json',
