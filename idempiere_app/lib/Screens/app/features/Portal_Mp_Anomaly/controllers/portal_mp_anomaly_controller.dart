...
class PortalMpAnomalyController extends GetxController {
  ...
  LitNcJson _trxDesktop = LitNcJson(records: []);
  ...
  Future<void> getAnomaliesDesktop() async {
    _desktopDataAvailable.value = false;
    var notificationFilter = "";
    if (Get.arguments != null) {
      if (Get.arguments['notificationId'] != null) {
        notificationFilter =
            " and LIT_NC_ID eq ${Get.arguments['notificationId']}";
        Get.arguments['notificationId'] = null;
      }
    }
    // ignore: unused_local_variable
    ...
    final protocol = GetStorage().read('protocol');
    var url = Uri.parse(
        '$protocol://$ip/api/v1/models/lit_mobile_nc_v?\$filter= C_BPartner_ID eq $businessPartnerId and AD_Client_ID eq ${GetStorage().read("clientid")}$notificationFilter$searchFilter&\$skip=${(desktopPagesCount.value - 1) * 100}');
    var response = await http.get(
      url,
      headers: <String, String>{
        'Content-Type': 'application/json',
    ...
    if (response.statusCode == 200) {
      ...
      for (var i = 0; i < _trxDesktop.records!.length; i++) {
        headerRows.add(DataRow(selected: false, cells: <DataCell>[
          DataCell(
            Text(_trxDesktop.records![i].mProductID?.identifier ?? 'N/A'),
          ),
          DataCell(
            Text(_trxDesktop.records![i].dateDoc ?? 'N/A'),
          ),
          DataCell(
            Text(_trxDesktop.records![i].lITNCFaultTypeID?.identifier ?? 'N/A'),
          ),
          DataCell(
            Text(_trxDesktop.records![i].ncDescription ?? 'N/A'),
          ),
          DataCell(
            SingleChildScrollView(
                child: Text(
                    _trxDesktop.records![i].cLocationID?.identifier ?? 'N/A')),
          ),
        ]));
      }
      //print(trx.records!.length);
