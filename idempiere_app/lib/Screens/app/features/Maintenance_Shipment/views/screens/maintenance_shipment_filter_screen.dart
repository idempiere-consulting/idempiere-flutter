...
class MaintenanceFilterShipment extends StatefulWidget {
  const MaintenanceFilterShipment({Key? key}) : super(key: key);

  @override
  State<MaintenanceFilterShipment> createState() =>
      _MaintenanceFilterShipmentState();
}

class _MaintenanceFilterShipmentState extends State<MaintenanceFilterShipment> {
  applyFilters() {
    ...
    if (dateStartFieldController.text != "") {
      try {
        ...
        Get.find<MaintenanceShipmentController>().dateStartFilter =
            " and MovementDate ge '${DateFormat('yyyy-MM-dd').format(date)} 00:00:00'";
        Get.find<MaintenanceShipmentController>().dateStartValue.value =
    ...
    if (dateEndFieldController.text != "") {
      try {
        ...
        Get.find<MaintenanceShipmentController>().dateEndFilter =
            " and MovementDate le '${DateFormat('yyyy-MM-dd').format(date)} 23:59:59'";
        Get.find<MaintenanceShipmentController>().dateEndValue.value =
    ...
    if (docNoFieldController.text != "") {
      Get.find<MaintenanceShipmentController>().docNoFilter =
          " and contains(DocumentNo,'${docNoFieldController.text}')";
    } else {
      Get.find<MaintenanceShipmentController>().docNoFilter = "";
    }

    Get.find<MaintenanceShipmentController>().businessPartnerId.value =
        businessPartnerId;

    if (businessPartnerId > 0) {
      Get.find<MaintenanceShipmentController>().businessPartnerName =
          bpSearchFieldController.text;
    } else {
      Get.find<MaintenanceShipmentController>().businessPartnerName = "";
    }

    Get.find<MaintenanceShipmentController>().docNoValue.value =
        docNoFieldController.text;

    Get.find<MaintenanceShipmentController>().getShipments();
    Get.back();
  }
