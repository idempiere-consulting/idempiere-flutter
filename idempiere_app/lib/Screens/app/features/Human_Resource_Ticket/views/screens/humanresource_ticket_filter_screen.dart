...
class HumanResourceFilterTicket extends StatefulWidget {
  const HumanResourceFilterTicket({Key? key}) : super(key: key);

  @override
  State<HumanResourceFilterTicket> createState() =>
      _HumanResourceFilterTicketState();
}

class _HumanResourceFilterTicketState extends State<HumanResourceFilterTicket> {
  applyFilters() {
    var inputFormat = DateFormat('dd/MM/yyyy');

    if (userId > 0) {
      Get.find<HumanResourceTicketController>().userFilter =
    ...
    if (dateStartFieldController.text != "") {
      try {
        ...
        Get.find<HumanResourceTicketController>().dateStartFilter =
            " and StartDate ge '${DateFormat('yyyy-MM-dd').format(date)} 00:00:00'";
        Get.find<HumanResourceTicketController>().dateStartValue.value =
    ...
    if (dateEndFieldController.text != "") {
      try {
        ...
        Get.find<HumanResourceTicketController>().dateEndFilter =
            " and StartDate le '${DateFormat('yyyy-MM-dd').format(date)} 23:59:59'";
        Get.find<HumanResourceTicketController>().dateEndValue.value =
    ...
    Get.find<HumanResourceTicketController>().userId.value = userId;

    if (userId > 0) {
      Get.find<HumanResourceTicketController>().userName =
  ...
  saveFilters() {
    ...
    if (userId > 0) {
      GetStorage()
          .write('TicketHR_userFilter', " and C_BPartner_ID eq $userId");
    } else {
      GetStorage().write('TicketHR_userFilter', "");
    }

    if (dateStartFieldController.text != "") {
      try {
        var date = inputFormat.parse(dateStartFieldController.text);
        GetStorage().write('TicketHR_dateStartFilter',
            " and StartDate ge '${DateFormat('yyyy-MM-dd').format(date)} 00:00:00'");
        GetStorage().write('TicketHR_dateStart', dateStartFieldController.text);
      } catch (e) {
        if (kDebugMode) {
          print(e);
        }
      ...
      GetStorage().write('TicketHR_dateStartFilter', "");
      GetStorage().write('TicketHR_dateStart', "");
    }
    ...
    if (dateEndFieldController.text != "") {
      try {
        ...
        GetStorage().write('TicketHR_dateEndFilter',
            " and StartDate le '${DateFormat('yyyy-MM-dd').format(date)} 23:59:59'");
        GetStorage().write('TicketHR_dateEnd', dateEndFieldController.text);
      } catch (e) {
        if (kDebugMode) {
          print(e);
        }
    ...
    GetStorage().write('TicketHR_userId', userId);
    GetStorage().write('TicketHR_userName', userSearchFieldController.text);
  }
