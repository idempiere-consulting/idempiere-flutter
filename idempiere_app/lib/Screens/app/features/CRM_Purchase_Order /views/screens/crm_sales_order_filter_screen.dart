...
class CRMFilterPurchaseOrder extends StatefulWidget {
  const CRMFilterPurchaseOrder({Key? key}) : super(key: key);

  @override
  State<CRMFilterPurchaseOrder> createState() => _CRMFilterPurchaseOrderState();
}

class _CRMFilterPurchaseOrderState extends State<CRMFilterPurchaseOrder> {
  applyFilters() {
    if (selectedUserRadioTile > 0) {
      if (selectedUserRadioTile == 2 && salesRepId > 0) {
        Get.find<CRMPurchaseOrderController>().userFilter =
    ...
    if (businessPartnerId > 0) {
      Get.find<CRMPurchaseOrderController>().businessPartnerFilter =
    ...
    if (docNoFieldController.text != "") {
      Get.find<CRMPurchaseOrderController>().docNoFilter =
          " and contains(DocumentNo,'${docNoFieldController.text}')";
    } else {
      Get.find<CRMPurchaseOrderController>().docNoFilter = "";
    }

    Get.find<CRMPurchaseOrderController>().selectedUserRadioTile.value =
        selectedUserRadioTile;

    Get.find<CRMPurchaseOrderController>().salesRepId = salesRepId;
    Get.find<CRMPurchaseOrderController>().salesRepName =
        salesRepFieldController.text;

    Get.find<CRMPurchaseOrderController>().businessPartnerId.value =
        businessPartnerId;

    if (businessPartnerId > 0) {
      Get.find<CRMPurchaseOrderController>().businessPartnerName =
          bpSearchFieldController.text;
    } else {
      Get.find<CRMPurchaseOrderController>().businessPartnerName = "";
    }
    Get.find<CRMPurchaseOrderController>().docNoValue.value =
