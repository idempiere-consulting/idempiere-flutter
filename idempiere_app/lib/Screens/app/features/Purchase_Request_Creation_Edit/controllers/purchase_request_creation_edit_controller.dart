...
class PurchaseRequestCreationEditController extends GetxController {
  ...
  getProdLines() async {
    ...
    var url = Uri.parse(
        '$protocol://$ip/api/v1/models/lit_mobile_reqline_v?\$filter= M_Requisition_ID eq ${args["requisitionId"]} and AD_Client_ID eq ${GetStorage().read("clientid")}');
  ...
  deletePurchaseRequestLines(int id) async {
    final ip = GetStorage().read('ip');
    String authorization = 'Bearer ${GetStorage().read('token')}';

    final protocol = GetStorage().read('protocol');
    var url = Uri.parse('$protocol://$ip/api/v1/models/M_RequisitionLine/$id');
    //print(msg);
  ...
  createPurchaseRequestLines(int id, double qty) async {
    final ip = GetStorage().read('ip');
    String authorization = 'Bearer ${GetStorage().read('token')}';
    final protocol = GetStorage().read('protocol');
    var url = Uri.parse(
        '$protocol://$ip/api/v1/windows/requisition/tabs/${"requisition".tr}/${args['requisitionId']}/${"requisition-line".tr}');
  ...
  editPurchaseRequestLines(int id, double qty) async {
    final ip = GetStorage().read('ip');
    String authorization = 'Bearer ${GetStorage().read('token')}';
    final protocol = GetStorage().read('protocol');
    var url = Uri.parse(
        '$protocol://$ip/api/v1/windows/sales-order/tabs/${"requisition-line".tr}/$id');
