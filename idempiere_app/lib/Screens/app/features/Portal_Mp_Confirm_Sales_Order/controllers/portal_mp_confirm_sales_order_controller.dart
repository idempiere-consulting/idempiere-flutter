...
class PortalMpConfirmSalesOrderController extends GetxController {
  ...
  var userFilter =
      GetStorage().read('PortalMPConfirmSalesOrder_userFilter') ?? "";
  var businessPartnerFilter =
      GetStorage().read('PortalMPConfirmSalesOrder_businessPartnerFilter') ??
          "";
  var docNoFilter =
      GetStorage().read('PortalMPConfirmSalesOrder_docNoFilter') ?? "";
  ...
  void onInit() {
    ...
    selectedUserRadioTile.value =
        GetStorage().read('PortalMPConfirmSalesOrder_selectedUserRadioTile') ??
            0;
    businessPartnerName =
        GetStorage().read('PortalMPConfirmSalesOrder_businessPartnerName') ??
            "";
    businessPartnerId.value =
        GetStorage().read('PortalMPConfirmSalesOrder_businessPartnerId') ?? 0;
    salesRepId = GetStorage().read('PortalMPConfirmSalesOrder_salesRepId') ?? 0;
    salesRepName =
        GetStorage().read('PortalMPConfirmSalesOrder_salesRepName') ?? "";
    docNoValue.value =
        GetStorage().read('PortalMPConfirmSalesOrder_docNo') ?? "";
  ...
  confirmOrder(int index) async {
    ...
    var url = Uri.parse(
        '$protocol://$ip/api/v1/models/C_Order/${_trx.records![index].id}');
    //print(msg);
  ...
  Future<void> signOrder(int index) async {
    final ip = GetStorage().read('ip');
    String authorization = 'Bearer ${GetStorage().read('token')}';

    final msg = jsonEncode({"name": "signature.jpg", "data": image64.value});

    final protocol = GetStorage().read('protocol');
    var url = Uri.parse(
        '$protocol://$ip/api/v1/models/C_Order/${_trx.records![index].id}/attachments');
  ...
  Future<void> getSalesOrders() async {
    ...
    final protocol = GetStorage().read('protocol');
    var url = Uri.parse(
        '$protocol://$ip/api/v1/models/lit_mobile_salesorder_v?\$filter= C_BPartner_ID eq $businessPartnerId and IsSoTrx eq Y and (DocStatus eq \'CO\' or  DocStatus eq \'DR\' or DocStatus eq \'PR\') and AD_Client_ID eq ${GetStorage().read("clientid")}${apiUrlFilter[filterCount]}$notificationFilter$userFilter$businessPartnerFilter$docNoFilter');
    var response = await http.get(
      url,
      headers: <String, String>{
        'Content-Type': 'application/json',
  ...
  Future<void> getPDFs(int index) async {
    final ip = GetStorage().read('ip');
    String authorization = 'Bearer ${GetStorage().read('token')}';
    final protocol = GetStorage().read('protocol');
    var url = Uri.parse(
  ...
  Future<void> openPDF(int index, String name) async {
    final ip = GetStorage().read('ip');
    String authorization = 'Bearer ${GetStorage().read('token')}';
    final protocol = GetStorage().read('protocol');
    var url = Uri.parse(
        '$protocol://$ip/api/v1/models/c_order/${_trx.records![index].id}/attachments/$name');
    var response = await http.get(
      url,
      headers: <String, String>{
        'Content-Type': 'application/json',
