...
class PortalMPFilterSalesOrder extends StatefulWidget {
  const PortalMPFilterSalesOrder({Key? key}) : super(key: key);

  @override
  State<PortalMPFilterSalesOrder> createState() =>
      _PortalMPFilterSalesOrderState();
}

class _PortalMPFilterSalesOrderState extends State<PortalMPFilterSalesOrder> {
  applyFilters() {
    if (selectedUserRadioTile > 0) {
      if (selectedUserRadioTile == 2 && salesRepId > 0) {
        Get.find<PortalMpConfirmSalesOrderController>().userFilter =
    ...
    if (businessPartnerId > 0) {
      Get.find<PortalMpConfirmSalesOrderController>().businessPartnerFilter =
    ...
    if (docNoFieldController.text != "") {
      Get.find<PortalMpConfirmSalesOrderController>().docNoFilter =
          " and contains(DocumentNo,'${docNoFieldController.text}')";
    } else {
      Get.find<PortalMpConfirmSalesOrderController>().docNoFilter = "";
    }

    Get.find<PortalMpConfirmSalesOrderController>()
        .selectedUserRadioTile
        .value = selectedUserRadioTile;

    Get.find<PortalMpConfirmSalesOrderController>().salesRepId = salesRepId;
    Get.find<PortalMpConfirmSalesOrderController>().salesRepName =
        salesRepFieldController.text;

    Get.find<PortalMpConfirmSalesOrderController>().businessPartnerId.value =
        businessPartnerId;

    if (businessPartnerId > 0) {
      Get.find<PortalMpConfirmSalesOrderController>().businessPartnerName =
          bpSearchFieldController.text;
    } else {
      Get.find<PortalMpConfirmSalesOrderController>().businessPartnerName = "";
    }
    Get.find<PortalMpConfirmSalesOrderController>().docNoValue.value =
        docNoFieldController.text;

    Get.find<PortalMpConfirmSalesOrderController>().getSalesOrders();
    Get.back();
  }
  ...
  saveFilters() {
    if (selectedUserRadioTile > 0) {
      if (selectedUserRadioTile == 2 && salesRepId > 0) {
        GetStorage().write('PortalMPConfirmSalesOrder_userFilter',
            " and SalesRep_ID eq $salesRepId");
      } else {
        GetStorage().write('PortalMPConfirmSalesOrder_userFilter',
            " and SalesRep_ID eq ${GetStorage().read('userId')}");
    ...
    if (businessPartnerId > 0) {
      GetStorage().write('PortalMPConfirmSalesOrder_businessPartnerFilter',
          " and C_BPartner_ID eq $businessPartnerId");
    } else {
      GetStorage().write('PortalMPConfirmSalesOrder_businessPartnerFilter', "");
    }
    ...
    if (docNoFieldController.text != "") {
      GetStorage().write('PortalMPConfirmSalesOrder_docNoFilter',
          " and contains(DocumentNo,'${docNoFieldController.text}')");
    } else {
      GetStorage().write('PortalMPConfirmSalesOrder_docNoFilter', "");
    }

    GetStorage().write('PortalMPConfirmSalesOrder_selectedUserRadioTile',
        selectedUserRadioTile);
    GetStorage().write('PortalMPConfirmSalesOrder_salesRepId', salesRepId);
    GetStorage().write(
        'PortalMPConfirmSalesOrder_salesRepName', salesRepFieldController.text);
    GetStorage().write(
        'PortalMPConfirmSalesOrder_businessPartnerId', businessPartnerId);
    GetStorage().write('PortalMPConfirmSalesOrder_businessPartnerName',
        bpSearchFieldController.text);
    GetStorage()
        .write('PortalMPConfirmSalesOrder_docNo', docNoFieldController.text);
  }
