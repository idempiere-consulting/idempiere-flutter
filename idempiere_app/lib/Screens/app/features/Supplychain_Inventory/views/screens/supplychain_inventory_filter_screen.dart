...
class SupplychainFilterInventory extends StatefulWidget {
  const SupplychainFilterInventory({Key? key}) : super(key: key);

  @override
  State<SupplychainFilterInventory> createState() =>
      _SupplychainFilterInventoryState();
}

class _SupplychainFilterInventoryState
    extends State<SupplychainFilterInventory> {
  applyFilters() {
    var inputFormat = DateFormat('dd/MM/yyyy');
    if (docNoFieldController.text != "") {
      Get.find<SupplychainInventoryController>().docNoFilter =
    ...
    if (warehouseId != "0") {
      Get.find<SupplychainInventoryController>().warehouseFilter =
          " and M_Warehouse_ID eq $warehouseId";
    } else {
      Get.find<SupplychainInventoryController>().docTypeFilter = "";
    }
    ...
    if (dateStartFieldController.text != "") {
      try {
        ...
        Get.find<SupplychainInventoryController>().dateStartFilter =
            " and MovementDate ge '${DateFormat('yyyy-MM-dd').format(date)} 00:00:00'";
        Get.find<SupplychainInventoryController>().dateStartValue.value =
    ...
    if (dateEndFieldController.text != "") {
      try {
        ...
        Get.find<SupplychainInventoryController>().dateEndFilter =
            " and MovementDate le '${DateFormat('yyyy-MM-dd').format(date)} 23:59:59'";
        Get.find<SupplychainInventoryController>().dateEndValue.value =
    ...
    Get.find<SupplychainInventoryController>().docNoValue.value =
        docNoFieldController.text;
    Get.find<SupplychainInventoryController>().warehouseId.value = warehouseId;
    Get.find<SupplychainInventoryController>().docTypeId.value = docTypeId;
  ...
  saveFilters() {
    var inputFormat = DateFormat('dd/MM/yyyy');
    if (docNoFieldController.text != "") {
      GetStorage().write('Inventory_docNoFilter',
          " and contains(DocumentNo,'${docNoFieldController.text}')");
    } else {
      GetStorage().write('Inventory_docNoFilter', "");
    }
    ...
    if (warehouseId != "0") {
      GetStorage().write(
          'Inventory_warehouseFilter', " and M_Warehouse_ID eq $warehouseId");
    } else {
      GetStorage().write('Inventory_warehouseFilter', "");
    }

    if (docTypeId != "0") {
      GetStorage()
          .write('Inventory_docTypeFilter', " and C_DocType_ID eq $docTypeId");
    } else {
      GetStorage().write('Inventory_docTypeFilter', "");
    }

    if (dateStartFieldController.text != "") {
      try {
        var date = inputFormat.parse(dateStartFieldController.text);
        GetStorage().write('Inventory_dateStartFilter',
            " and MovementDate ge '${DateFormat('yyyy-MM-dd').format(date)} 00:00:00'");
        GetStorage()
            .write('Inventory_dateStart', dateStartFieldController.text);
      } catch (e) {
        if (kDebugMode) {
          print(e);
        }
      ...
      GetStorage().write('Inventory_dateStartFilter', "");
      GetStorage().write('Inventory_dateStart', "");
    }
    ...
    if (dateEndFieldController.text != "") {
      try {
        ...
        GetStorage().write('Inventory_dateEndFilter',
            " and MovementDate le '${DateFormat('yyyy-MM-dd').format(date)} 23:59:59'");
        GetStorage().write('Inventory_dateEnd', dateEndFieldController.text);
      } catch (e) {
        if (kDebugMode) {
          print(e);
        }
      ...
      GetStorage().write('Inventory_dateEndFilter', "");
      GetStorage().write('Inventory_dateEnd', "");
    }

    GetStorage().write('Inventory_docNo', docNoFieldController.text);
    GetStorage().write('Inventory_warehouseId', warehouseId);
    GetStorage().write('Inventory_docTypeId', docTypeId);
  }
