...
class PurchaseEditRequest extends StatefulWidget {
  const PurchaseEditRequest({Key? key}) : super(key: key);

  @override
  State<PurchaseEditRequest> createState() => _PurchaseEditRequestState();
}

class _PurchaseEditRequestState extends State<PurchaseEditRequest> {
  ...
  Future<List<DTRecords>> getAllDocType() async {
    ...
    final protocol = GetStorage().read('protocol');
    var url = Uri.parse(
        '$protocol://$ip/api/v1/models/C_DocType?\$filter= DocBaseType eq \'POO\' and AD_Client_ID eq ${GetStorage().read('clientid')}');
