...
class PurchaseFilterRequest extends StatefulWidget {
  const PurchaseFilterRequest({Key? key}) : super(key: key);

  @override
  State<PurchaseFilterRequest> createState() => _PurchaseFilterRequestState();
}

class _PurchaseFilterRequestState extends State<PurchaseFilterRequest> {
  applyFilters() {
    if (selectedUserRadioTile > 0) {
      if (selectedUserRadioTile == 2 && salesRepId > 0) {
        Get.find<PurchaseRequestController>().userFilter =
    ...
    if (businessPartnerId > 0) {
      Get.find<PurchaseRequestController>().businessPartnerFilter =
    ...
    if (docNoFieldController.text != "") {
      Get.find<PurchaseRequestController>().docNoFilter =
          " and contains(DocumentNo,'${docNoFieldController.text}')";
    } else {
      Get.find<PurchaseRequestController>().docNoFilter = "";
    }

    Get.find<PurchaseRequestController>().selectedUserRadioTile.value =
        selectedUserRadioTile;

    Get.find<PurchaseRequestController>().salesRepId = salesRepId;
    Get.find<PurchaseRequestController>().salesRepName =
        salesRepFieldController.text;

    Get.find<PurchaseRequestController>().businessPartnerId.value =
        businessPartnerId;

    if (businessPartnerId > 0) {
      Get.find<PurchaseRequestController>().businessPartnerName =
          bpSearchFieldController.text;
    } else {
      Get.find<PurchaseRequestController>().businessPartnerName = "";
    }
    Get.find<PurchaseRequestController>().docNoValue.value =
  ...
  saveFilters() {
    if (selectedUserRadioTile > 0) {
      if (selectedUserRadioTile == 2 && salesRepId > 0) {
        GetStorage().write(
            'PurchaseRequest_userFilter', " and SalesRep_ID eq $salesRepId");
      } else {
        GetStorage().write('PurchaseRequest_userFilter',
            " and SalesRep_ID eq ${GetStorage().read('userId')}");
      }
    } else {
      GetStorage().write('PurchaseRequest_userFilter', "");
    ...
    if (businessPartnerId > 0) {
      GetStorage().write('PurchaseRequest_businessPartnerFilter',
          " and C_BPartner_ID eq $businessPartnerId");
    } else {
      GetStorage().write('PurchaseRequest_businessPartnerFilter', "");
    }
    ...
    if (docNoFieldController.text != "") {
      GetStorage().write('PurchaseRequest_docNoFilter',
          " and contains(DocumentNo,'${docNoFieldController.text}')");
    } else {
      GetStorage().write('PurchaseRequest_docNoFilter', "");
    }
    ...
    GetStorage()
        .write('PurchaseRequest_selectedUserRadioTile', selectedUserRadioTile);
    GetStorage().write('PurchaseRequest_salesRepId', salesRepId);
    GetStorage()
        .write('PurchaseRequest_salesRepName', salesRepFieldController.text);
    GetStorage().write('PurchaseRequest_businessPartnerId', businessPartnerId);
    GetStorage().write(
        'PurchaseRequest_businessPartnerName', bpSearchFieldController.text);
    GetStorage().write('PurchaseRequest_docNo', docNoFieldController.text);
  }
