...
class PurchaseOrderController extends GetxController {
  ...
  var userFilter = GetStorage().read('PurchaseOrder_userFilter') ?? "";
  var businessPartnerFilter =
      GetStorage().read('PurchaseOrder_businessPartnerFilter') ?? "";
  var docNoFilter = GetStorage().read('PurchaseOrder_docNoFilter') ?? "";
  ...
  void onInit() {
    ...
    selectedUserRadioTile.value =
        GetStorage().read('PurchaseOrder_selectedUserRadioTile') ?? 0;
    businessPartnerName =
        GetStorage().read('PurchaseOrder_businessPartnerName') ?? "";
    businessPartnerId.value =
        GetStorage().read('PurchaseOrder_businessPartnerId') ?? 0;
    salesRepId = GetStorage().read('PurchaseOrder_salesRepId') ?? 0;
    salesRepName = GetStorage().read('PurchaseOrder_salesRepName') ?? "";
    docNoValue.value = GetStorage().read('PurchaseOrder_docNo') ?? "";
    getSalesOrders();
    getADUserID();
    setConnect();
  }
  ...
  Future<void> getSalesOrders() async {
    ...
    final protocol = GetStorage().read('protocol');
    var url = Uri.parse(
        '$protocol://$ip/api/v1/models/c_order?\$filter= IsSoTrx eq N and DocStatus neq \'VO\' and DocStatus neq \'CO\' and AD_Client_ID eq ${GetStorage().read("clientid")}${apiUrlFilter[filterCount]}$notificationFilter$userFilter$businessPartnerFilter$docNoFilter&\$orderby= DateOrdered desc');
    var response = await http.get(
      url,
      headers: <String, String>{
        'Content-Type': 'application/json',
