...
class PortalMpTrainingCourseCourseListController extends GetxController {
  ...
  Future<void> getCourseListsDesktop() async {
    ...
    final protocol = GetStorage().read('protocol');
    var url = Uri.parse(
        '$protocol://$ip/api/v1/models/MP_Maintain?\$filter= C_BPartner_ID eq $businessPartnerId and WindowType eq \'T\' and AD_Client_ID eq ${GetStorage().read("clientid")}$notificationFilter$searchFilter&\$skip=${(desktopPagesCount.value - 1) * 100}');
    var response = await http.get(
      url,
      headers: <String, String>{
    ...
    if (response.statusCode == 200) {
      ...
      for (var i = 0; i < _trxDesktop.records!.length; i++) {
        headerRows.add(DataRow(selected: false, cells: <DataCell>[
          DataCell(Text(_trxDesktop.records![i].documentNo ?? '??')),
