...
class _PortalMPFilterSalesOrderState extends State<PortalMPFilterSalesOrder> {
  ...
  saveFilters() {
    if (selectedUserRadioTile > 0) {
      if (selectedUserRadioTile == 2 && salesRepId > 0) {
        GetStorage().write(
            'PortalMPSalesOrder_userFilter', " and SalesRep_ID eq $salesRepId");
      } else {
        GetStorage().write('PortalMPSalesOrder_userFilter',
            " and SalesRep_ID eq ${GetStorage().read('userId')}");
      }
    } else {
      GetStorage().write('PortalMPSalesOrder_userFilter', "");
    ...
    if (businessPartnerId > 0) {
      GetStorage().write('PortalMPSalesOrder_businessPartnerFilter',
          " and C_BPartner_ID eq $businessPartnerId");
    } else {
      GetStorage().write('PortalMPSalesOrder_businessPartnerFilter', "");
    }
    ...
    if (docNoFieldController.text != "") {
      GetStorage().write('PortalMPSalesOrder_docNoFilter',
          " and contains(DocumentNo,'${docNoFieldController.text}')");
    } else {
      GetStorage().write('PortalMPSalesOrder_docNoFilter', "");
    }
    ...
    GetStorage().write(
        'PortalMPSalesOrder_selectedUserRadioTile', selectedUserRadioTile);
    GetStorage().write('PortalMPSalesOrder_salesRepId', salesRepId);
    GetStorage()
        .write('PortalMPSalesOrder_salesRepName', salesRepFieldController.text);
    GetStorage()
        .write('PortalMPSalesOrder_businessPartnerId', businessPartnerId);
    GetStorage().write(
        'PortalMPSalesOrder_businessPartnerName', bpSearchFieldController.text);
    GetStorage().write('PortalMPSalesOrder_docNo', docNoFieldController.text);
  }
