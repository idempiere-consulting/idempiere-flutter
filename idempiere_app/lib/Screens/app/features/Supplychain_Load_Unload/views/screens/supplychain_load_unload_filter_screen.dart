...
class SupplychainFilterLoadUnload extends StatefulWidget {
  const SupplychainFilterLoadUnload({Key? key}) : super(key: key);

  @override
  State<SupplychainFilterLoadUnload> createState() =>
      _SupplychainFilterLoadUnloadState();
}

class _SupplychainFilterLoadUnloadState
    extends State<SupplychainFilterLoadUnload> {
  applyFilters() {
    var inputFormat = DateFormat('dd/MM/yyyy');
    if (selectedUserRadioTile > 0) {
      if (selectedUserRadioTile == 2 && salesRepId > 0) {
        Get.find<SupplychainLoadUnloadController>().userFilter =
    ...
    if (businessPartnerId > 0) {
      Get.find<SupplychainLoadUnloadController>().businessPartnerFilter =
    ...
    if (docNoFieldController.text != "") {
      Get.find<SupplychainLoadUnloadController>().docNoFilter =
    ...
    if (descriptionFieldController.text != "") {
      Get.find<SupplychainLoadUnloadController>().descriptionFilter =
    ...
    if (dateStartFieldController.text != "") {
      try {
        ...
        Get.find<SupplychainLoadUnloadController>().dateStartFilter =
            " and DateInvoiced ge '${DateFormat('yyyy-MM-dd').format(date)} 00:00:00'";
        Get.find<SupplychainLoadUnloadController>().dateStartValue.value =
    ...
    if (dateEndFieldController.text != "") {
      try {
        ...
        Get.find<SupplychainLoadUnloadController>().dateEndFilter =
            " and DateInvoiced le '${DateFormat('yyyy-MM-dd').format(date)} 23:59:59'";
        Get.find<SupplychainLoadUnloadController>().dateEndValue.value =
    ...
    Get.find<SupplychainLoadUnloadController>().selectedUserRadioTile.value =
        selectedUserRadioTile;

    Get.find<SupplychainLoadUnloadController>().salesRepId = salesRepId;
    Get.find<SupplychainLoadUnloadController>().salesRepName =
        salesRepFieldController.text;

    Get.find<SupplychainLoadUnloadController>().businessPartnerId.value =
        businessPartnerId;

    if (businessPartnerId > 0) {
      Get.find<SupplychainLoadUnloadController>().businessPartnerName =
          bpSearchFieldController.text;
    } else {
      Get.find<SupplychainLoadUnloadController>().businessPartnerName = "";
    }
    Get.find<SupplychainLoadUnloadController>().docNoValue.value =
        docNoFieldController.text;
    Get.find<SupplychainLoadUnloadController>().description.value =
  ...
  saveFilters() {
    ...
    if (selectedUserRadioTile > 0) {
      if (selectedUserRadioTile == 2 && salesRepId > 0) {
        GetStorage()
            .write('LoadUnload_userFilter', " and SalesRep_ID eq $salesRepId");
      } else {
        GetStorage().write('LoadUnload_userFilter',
            " and SalesRep_ID eq ${GetStorage().read('userId')}");
      }
    } else {
      GetStorage().write('LoadUnload_userFilter', "");
    ...
    if (businessPartnerId > 0) {
      GetStorage().write('LoadUnload_businessPartnerFilter',
          " and C_BPartner_ID eq $businessPartnerId");
    } else {
      GetStorage().write('LoadUnload_businessPartnerFilter', "");
    }
    ...
    if (docNoFieldController.text != "") {
      GetStorage().write('LoadUnload_docNoFilter',
          " and contains(DocumentNo,'${docNoFieldController.text}')");
    } else {
      GetStorage().write('LoadUnload_docNoFilter', "");
    }
    ...
    if (descriptionFieldController.text != "") {
      GetStorage().write('LoadUnload_descriptionFilter',
          " and contains(tolower(Description),'${descriptionFieldController.text}')");
    } else {
      GetStorage().write('LoadUnload_descriptionFilter', "");
    }
    ...
    if (dateStartFieldController.text != "") {
      try {
        var date = inputFormat.parse(dateStartFieldController.text);
        GetStorage().write('LoadUnload_dateStartFilter',
            " and DateInvoiced ge '${DateFormat('yyyy-MM-dd').format(date)} 00:00:00'");
        GetStorage()
            .write('LoadUnload_dateStart', dateStartFieldController.text);
      } catch (e) {
        if (kDebugMode) {
          print(e);
        }
      ...
      GetStorage().write('LoadUnload_dateStartFilter', "");
      GetStorage().write('LoadUnload_dateStart', "");
    }
    ...
    if (dateEndFieldController.text != "") {
      try {
        ...
        GetStorage().write('LoadUnload_dateEndFilter',
            " and DateInvoiced le '${DateFormat('yyyy-MM-dd').format(date)} 23:59:59'");
        GetStorage().write('LoadUnload_dateEnd', dateEndFieldController.text);
      } catch (e) {
        if (kDebugMode) {
          print(e);
        }
    ...
    GetStorage()
        .write('LoadUnload_selectedUserRadioTile', selectedUserRadioTile);
    GetStorage().write('LoadUnload_salesRepId', salesRepId);
    GetStorage().write('LoadUnload_salesRepName', salesRepFieldController.text);
    GetStorage().write('LoadUnload_businessPartnerId', businessPartnerId);
    GetStorage()
        .write('LoadUnload_businessPartnerName', bpSearchFieldController.text);
    GetStorage().write('LoadUnload_docNo', docNoFieldController.text);
    GetStorage()
        .write('LoadUnload_description', descriptionFieldController.text);
  }
